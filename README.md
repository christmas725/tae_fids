...
- 휴대폰 세로 화면에서는 상단 공항·출도착 전환 영역을 압축하고 `시각 / 편명 / 목적지·출발지 / 게이트·수하물 / 현황` 5열로 표시합니다.
- 휴대폰 가로 화면은 좌측 레일형 FIDS를 유지하면서 짧은 화면 높이에서도 15행과 상태 바가 한 화면에 들어오도록 조정합니다.
- Galaxy Fold 등 펼친 폴더블·터치 태블릿은 가로형 상단 헤더와 전체 운항표를 사용합니다.
- `100dvh`, safe-area, standalone display를 적용해 브라우저 주소 표시줄과 노치·상태 표시줄 영역을 처리합니다.
- 홈 화면 설치용 192px·512px·maskable·Apple Touch 아이콘을 제공합니다.
- 서비스 워커는 앱 셸과 아이콘만 캐시하며 `/api/` 실시간 운항정보는 캐시하지 않습니다.
- 오프라인에서는 마지막 실시간 데이터를 보여주는 대신 연결 대기 안내 화면을 표시합니다.
...
- 2026-09-02 실제 운항편 `KE1432`와 `AF7897`, `DL7846`, `GA9998`, `VS5534`가 마스터 편명 기준으로 묶이는 것을 Production에서 확인
...
1. 대구공항 홈페이지 응답 변경 감시 및 fallback 보강
2. 기존 대구공항 자동 안내방송/TTS 상태 머신 연결
